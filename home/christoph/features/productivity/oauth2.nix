...
rec {
  work = rec {
    authorize = pkgs.writeShellScriptBin "authorize_work_mail" "${pwd_cmd} --authorize";
    pwd_cmd = "${lib.getExe mutt_oauth2} --client-id '08162f7c-0fd2-4200-a84a-f25a4db0b584' --client-secret 'TxRBilcHdC6WGBee]fs?QR:SJ8nI[g82' --provider microsoft --email chrisjab@ad.helsinki.fi --authflow localhostauthcode ~/Mail/work-auth.gpg";
  };
  gmail = rec {
    authorize = pkgs.writeShellScriptBin "authorize_gmail" "${pwd_cmd} --authorize";
    pwd_cmd = "${lib.getExe mutt_oauth2} --client-id '08162f7c-0fd2-4200-a84a-f25a4db0b584' --client-secret 'TxRBilcHdC6WGBee]fs?QR:SJ8nI[g82' --provider google --email christoph.jabs@gmail.com --authflow localhostauthcode ~/Mail/gmail-auth.gpg";
  };
  mutt_oauth2 =
    pkgs.writers.writePython3Bin "mutt_oauth2.py" { }
      # python
      ''
        ...
        maintained in an encrypted TOKENFILE.  Run with "--verbose --authorize
        --encryption-pipe 'foo@bar.org'" to get started or whenever all tokens have
        expired, optionally with "--authflow" to override the default authorization
        flow.  To truly start over from scratch, first delete TOKENFILE. Use "--verbose
        --test" to test the IMAP/POP/SMTP endpoints.
        '''')
        ...
        ap.add_argument('--decryption-pipe', type=shlex.split, default=DECRYPTION_PIPE,
                        help='decryption command (string), reads from stdin and '
                        'writes to stdout, default: "{}"'.format(
                            " ".join(DECRYPTION_PIPE)))
        ap.add_argument('--encryption-pipe', type=shlex.split, default=ENCRYPTION_PIPE,
                        help='encryption command (string), reads from stdin and '
                        'writes to stdout, suggested: "{}"'.format(
        ...
        if path.exists():
            if 0o777 & path.stat().st_mode != 0o600:
                sys.exit('Token file has unsafe mode. '
                         'Suggest deleting and starting over.')
            try:
                sub = subprocess.run(DECRYPTION_PIPE, check=True,
                                     input=path.read_bytes(), capture_output=True)
                token = json.loads(sub.stdout)
            except subprocess.CalledProcessError:
                sys.exit('Difficulty decrypting token file. Is your decryption agent '
        ...
        def writetokenfile():
            ...
            sub2 = subprocess.run(ENCRYPTION_PIPE, check=True,
        ...
        if not token:
            ...
            token['authflow'] = args.authflow or input(
                ...
                '("authcode" or "localhostauthcode" or "devicecode"): '
            )
        ...
        if token['registration'] not in registrations:
            sys.exit(f'ERROR: Unknown registration "{token["registration"]}". '
                     f'Delete token file and start over.')
        ...
        def access_token_valid():
            ''''Returns True when stored access token exists
            and is still valid at this time.''''
            token_exp = token['access_token_expiration']
            return token_exp and datetime.now() < datetime.fromisoformat(token_exp)


        def update_tokens(r):
            ''''Takes a response dictionary, extracts tokens out of it,
            and updates token file.''''
            token['access_token'] = r['access_token']
            token['access_token_expiration'] = \
                (datetime.now() + timedelta(seconds=int(r['expires_in']))).isoformat()
            if 'refresh_token' in r:
                token['refresh_token'] = r['refresh_token']
            writetokenfile()
            if args.verbose:
                print(f'NOTICE: Obtained new access token, '
                      f'expires {token["access_token_expiration"]}.')
        ...
        if args.authorize:
            ...
            if authflow in ('authcode', 'localhostauthcode'):
                ...
                if authflow == 'localhostauthcode':
                    ...
                    # Probably should edit the port number into the actual
                    # redirect URL.
                ...
                if authflow == 'authcode':
                    authcode = input('Visit displayed URL to retrieve authorization '
                                     'code. Enter code from server (might be in '
                                     'browser address bar): ')
                else:
                    print('Visit displayed URL to authorize this application. '
                    ...
                    class MyHandler(http.server.BaseHTTPRequestHandler):
                        ...
                        def do_GET(self):
                            ...
                            self.wfile.write(b'<html><head><title>'
                                             b'Authorization result</title></head>')
                            self.wfile.write(
                                b'<body><p>Authorization redirect completed. You may '
                                b'close this window.</p></body></html>'
                            )
                    with http.server.HTTPServer(
                        ('127.0.0.1', listen_port), MyHandler
                    ) as httpd:
                        try:
                ...
                for k in 'response_type', 'login_hint', 'code_challenge', \
                ...
                while True:
                    ...
                    try:
                        ...
                        # Not actually always an error, might just mean
                        # "keep trying..."
                        response = err
                    response = response.read()
                    if args.debug:
                ...
                sys.exit(f'ERROR: Unknown OAuth2 flow "{token["authflow"]}. '
                         f'Delete token file and start over.')
        ...
        if not access_token_valid():
            ...
            if 'error' in response:
                ...
                if 'error_description' in response:
                    print(response['error_description'])
                print('Perhaps refresh token invalid. Try running once with '
                      '"--authorize"')
                sys.exit(1)
            update_tokens(response)
        ...
        if not access_token_valid():
            sys.exit('ERROR: No valid access token. '
        ...
        def build_sasl_string(user, host, port, bearer_token):
            ''''Build appropriate SASL string, which depends on cloud server's
            supported SASL method.''''
            if registration['sasl_method'] == 'OAUTHBEARER':
                return f'n,a={user},\1host={host}\1port={port}\1auth=Bearer ' \
                       f'{bearer_token}\1\1'
        ...
        if args.test:
            ...
            sasl_string = build_sasl_string(token['email'],
                                            registration['imap_endpoint'], 993,
                                            token['access_token'])
            if args.debug:
                imap_conn.debug = 4
            try:
                ...
                # Microsoft has a bug wherein a mismatch between username and token
                # can still report a successful login... (Try a consumer login with
                # the token from a work/school account.)
                # Fortunately subsequent commands fail with an error. Thus we follow
                # AUTH with another IMAP command before reporting success.
                imap_conn.list()
                if args.verbose:
            ...
            try:
                ...
                print('POP authentication FAILED (does your account allow POP?):',
                      e.args[0].decode())
                errors = True
            ...
            sasl_string = build_sasl_string(token['email'],
                                            registration['smtp_endpoint'], 587,
                                            token['access_token'])
            smtp_conn.ehlo('test')
            smtp_conn.starttls()
            smtp_conn.ehlo('test')
